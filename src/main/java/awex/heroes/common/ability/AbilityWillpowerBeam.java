        ...
        if (phase == TickEvent.Phase.END && SHData.AIMING.get(entity)) {
            ...
            if (entity.worldObj.isRemote) {
                ...
                for(double point = 0.0D; point <= d0; point += 0.15D) {
                    ...
                    ModParticles.spawnParticle(ModParticleType.WILL, vec3.xCoord + (Math.random() - 0.5D) * (double)spread, vec3.yCoord + (Math.random() - 0.5D) * (double)spread, vec3.zCoord + (Math.random() - 0.5D) * (double)spread, (Math.random() - 0.5D) * (double)motion, (Math.random() - 0.5D) * (double)motion, (Math.random() - 0.5D) * (double)motion);
                }
