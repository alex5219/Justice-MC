...
public class AbilityTenacity extends Ability<IAbility> {
    public AbilityTenacity(int tier) {
        super(tier);
    }
    ...
    public void onUpdate(EntityLivingBase entity, Hero hero, IAbility instance, TickEvent.Phase phase) {
        ...
        if (phase == TickEvent.Phase.END && entity.getHealth() < entity.getMaxHealth() && (Short) SHData.TIME_SINCE_DAMAGED.get(entity) > 200) {
            entity.heal(1.0F);
        }
    }
}
