...
public class AbilityFearToxin extends Ability<IAiming> {
    AbilityFearToxin(int tier) {
        super(tier);
    }
        ...
        if (phase == TickEvent.Phase.END && SHData.AIMING.get(entity)) {
            ...
            if (rayTrace != null) {
                if (rayTrace.entityHit != null) {
                    ...
                    for(d1 = entity.posZ - rayTrace.entityHit.posZ; d0 * d0 + d1 * d1 < 1.0E-4D; d1 = (Math.random() - Math.random()) * 0.01D) {
                        d0 = (Math.random() - Math.random()) * 0.5D;
                    }
                    if (rayTrace.entityHit.attackEntityFrom(ModDamageSources.causeCollisionDamage(entity), 14.0F)) {
                        if (rayTrace.entityHit instanceof EntityLivingBase) {
                            ((EntityLivingBase)rayTrace.entityHit).knockBack(rayTrace.entityHit, 0.0F, d0 * 50.0D, 1 * 50.0D);
                            ((EntityLivingBase) rayTrace.entityHit).addPotionEffect(new PotionEffect(Potion.confusion.id, 20, 1));
                            ((EntityLivingBase) rayTrace.entityHit).addPotionEffect(new PotionEffect(Potion.poison.id, 20, 1));
                            ((EntityLivingBase) rayTrace.entityHit).addPotionEffect(new PotionEffect(Potion.blindness.id, 20, 1));
            ...
            if (entity.worldObj.isRemote) {
                ...
                for(double point = 0.0D; point <= d0; point += 0.15D) {
                    ...
                    ModParticles.spawnParticle(ModParticleType.FEAR, vec3.xCoord + (Math.random() - 0.5D) * (double)spread, vec3.yCoord + (Math.random() - 0.5D) * (double)spread, vec3.zCoord + (Math.random() - 0.5D) * (double)spread, (Math.random() - 0.5D) * (double)motion, (Math.random() - 0.5D) * (double)motion, (Math.random() - 0.5D) * (double)motion);
                }
